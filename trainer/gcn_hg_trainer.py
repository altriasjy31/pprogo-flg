...
class HGAT_Trainer(object):
    ...
    def __init__(self, args):
        ...
        self.dataset = DBLPDataset('data', args['dataset_name'], 'GCN_hg', args['batch_size'], self.device)
        self.g = self.dataset.g.to(self.device)
        self.model = RGCN(in_dim=self.dataset.feature_dim, hidden_dim=128, out_dim=self.dataset.go_num, etypes=self.g.canonical_etypes, num_bases=6)
        ...
        with torch.no_grad():
            ...
            for mode in modes:
                ...
                # for i, (seed, subgraph_protein_nodes, subgraph) in enumerate(loader_tqdm):
                    with autocast():
                        seed = seed['protein']
                        subgraph = dgl.node_subgraph(self.g, ntype_mp_name_input_nodes_dict, relabel_nodes=True)
                        # subgraph = blocks_to_hetero_graph(ntype_mp_name_block_dict)
                        label = self.dataset.get_label(seed.cpu().numpy().tolist())
