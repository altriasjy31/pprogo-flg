    ...
    **Authors:** Michael Schlichtkrull, Thomas N. Kipf, Peter Bloem, Rianne van den Berg, Ivan Titov, Max Welling
    ...
    def __init__(self, in_dim,
        ...
        self.fc1 = nn.ModuleDict({
            ...
            # for ntype in ntypes
            'protein': nn.Linear(in_dim, hidden_dim),
            'go_annotation': nn.Linear(out_dim, hidden_dim)
        })
        self.fc2 = nn.ModuleDict({
            'protein': nn.Linear(hidden_dim, out_dim),
            'go_annotation': nn.Linear(hidden_dim, out_dim)
        })
    ...
    def l2_norm(self, x):
        ...
        return x / (th.max(th.norm(x, dim=1, keepdim=True), self.epsilon))
    ...
    def forward(self, hg, h_dict):
        ...
        for ntype, feat in h_dict.items():
                h_dict[ntype] = self.drop(self.l2_norm(self.fc1[ntype](feat.to(th.float32))))
        hg.ndata['h'] = h_dict
        ...
        for ntype, feat in h_dict.items():
                h_dict[ntype] = self.fc2[ntype](feat.to(th.float32))
...
class RelGraphConvLayer(nn.Module):
    ...
    def forward(self, g, inputs):
        ...
        #     hs[ntype] = hs[ntype] + g.ndata['h'][ntype]
